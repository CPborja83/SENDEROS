(() => {
  ...
  // Cámara: SIN retraso. El bebé queda siempre centrado.
  const camera=new B.FreeCamera('camera',new B.Vector3(0,6.2,15),scene);
  camera.inputs.clear();
  camera.minZ=.08; camera.maxZ=150; camera.fov=.64;
  ...
  window.addEventListener('blur',()=>{keys.clear();touchDirections.clear();runTouch=false;});
  window.addEventListener('pointerup',()=>touchDirections.clear());
  window.addEventListener('pointercancel',()=>touchDirections.clear());
  ...
  document.querySelectorAll('[data-touch]').forEach(btn=>{
    ...
    const down=e=>{
      ...
      touchDirections.add(dir);
      try{btn.setPointerCapture(e.pointerId)}catch(_){}
    };
    const up=e=>{
      e.preventDefault();
      touchDirections.delete(dir);
    };
  ...
  async function loadBaby(){
    try{
      loadingText.textContent='Cargando Bebé Azul 3D…';
      ...
      result.meshes.filter(m=>!m.parent || !importedSet.has(m.parent)).forEach(m=>m.parent=modelPivot);
      ...
      // Normaliza el tamaño a ~2.3 m y coloca los pies sobre el piso.
      scene.render();
      ...
      modelReady=true;
      status.textContent='MODELO NUEVO listo · movimiento suave · cámara centrada';
  ...
  function pressed(dir){
    return keys.has(dir) || touchDirections.has(dir);
  }
  ...
  scene.onBeforeRenderObservable.add(()=>{
    ...
    const running=keys.has('Shift') || keys.has('ShiftLeft') || keys.has('ShiftRight') || runTouch;
    const maxSpeed=running?3.0:1.70;
    ...
    // Aceleración y frenado suaves: evita los saltos laterales.
    ...
    avatar.position.x+=velocity.x*dt;
    avatar.position.z+=velocity.z*dt;

    avatar.position.x=B.Scalar.Clamp(avatar.position.x,-5.25,5.25);
    avatar.position.z=B.Scalar.Clamp(avatar.position.z,-20.0,20.0);
    ...
    if(moving){
      ...
      avatar.rotation.y+=diff*(1-Math.exp(-10*dt));

      const gait=t*(running?9.0:6.6);
      bob=B.Scalar.Lerp(bob,Math.abs(Math.sin(gait))*.035,1-Math.exp(-10*dt));
      modelPivot.rotation.z=B.Scalar.Lerp(modelPivot.rotation.z,Math.sin(gait)*.018,1-Math.exp(-9*dt));
    }else{
      bob=B.Scalar.Lerp(bob,0,1-Math.exp(-10*dt));
      modelPivot.rotation.z=B.Scalar.Lerp(modelPivot.rotation.z,0,1-Math.exp(-10*dt));
    }

    avatar.position.y=bob;

    // Cámara anclada al personaje: NO se queda atrás y NO permite que salga de pantalla.
    camera.position.x=avatar.position.x;
    camera.position.y=6.15;
    camera.position.z=avatar.position.z+7.0;
    camera.setTarget(new B.Vector3(avatar.position.x,1.10,avatar.position.z));
