...
function updateDarkness(){
  ...
  // La cámara observa muchas cuadras; las nubes físicas son las que ocultan el mapa.
  scene.fogStart=34.0;
  scene.fogEnd=B.Scalar.Clamp(92.0-low*12.0,72.0,92.0);

  cloudDark.alpha=B.Scalar.Lerp(.68,.84,low);
  cloudMid.alpha=B.Scalar.Lerp(.58,.76,low);
  cloudLight.alpha=B.Scalar.Lerp(.36,.52,low);
...
async function loadCharacter(){
  try{
    ...
    setStatus(`MAPA GRANDE v16 · ${groundBoneMode} · ${boneNames}`);
...
const baseCamY=42.0;
const baseCamZ=player.position.z+15.0;
