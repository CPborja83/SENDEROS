...
function makeMistPatch(x,z,id){
  ...
  for(let i=0;i<4;i++){
    ...
    plane.parent=root;
    plane.position.set((rand()-.5)*2.0,.42+rand()*1.05,(rand()-.5)*2.0);
    plane.material=mistMat;
    plane.billboardMode=B.Mesh.BILLBOARDMODE_Y;
    plane.isPickable=false;
    plane.visibility=.80+rand()*.14;
    plane.metadata={baseY:plane.position.y,phase:rand()*Math.PI*2,baseScale:0.88+rand()*.30};
    plane.scaling.set(plane.metadata.baseScale,plane.metadata.baseScale,1);
  }
...
let mistId=0;
for(let z=-55;z<=22;z+=5.0){
  for(let x=-27;x<=27;x+=5.0){
    if(rand()<.05)continue;
    makeMistPatch(x+(rand()-.5)*.9,z+(rand()-.5)*.9,mistId++);
  }
}
...
// Huella despejada optimizada: al registrar un punto se limpian las tinieblas cercanas UNA vez.
const clearedTrail=[];
let lastTrailPoint=null;
let mistAnimFrame=0;

function clearMistNear(x,z,radius=6.10){
  const r2=radius*radius;
  for(const patch of mistPatches){
    if(!patch || patch.metadata?.cleared)continue;
    const dx=patch.metadata.baseX-x;
    const dz=patch.metadata.baseZ-z;
    if(dx*dx+dz*dz<r2){
      patch.metadata.cleared=true;
...
function recordClearedTrail(){
  ...
  if(!lastTrailPoint || B.Vector3.DistanceSquared(p,lastTrailPoint)>.48*.48){
    clearedTrail.push({x:p.x,z:p.z});
    if(clearedTrail.length>400)clearedTrail.shift();
    lastTrailPoint=p.clone();
    clearMistNear(p.x,p.z,6.10);
  }
}

function puffInsideClearedTrail(wx,wz){
  // Conservada por compatibilidad con otras funciones; ya no se usa en el bucle por cuadro.
  for(let i=clearedTrail.length-1;i>=0;i--){
    const p=clearedTrail[i];
    const dx=wx-p.x,dz=wz-p.z;
    if(dx*dx+dz*dz<6.10*6.10)return true;
  }
  return false;
}
...
function updateCloudClearing(){
  recordClearedTrail();
  // Limpieza inmediata alrededor del personaje para que jamás quede oculto.
  clearMistNear(player.position.x,player.position.z,6.40);
  mistAnimFrame++;

  // Animar vapor solo cada 3 cuadros y solo en un radio útil alrededor de la cámara/jugador.
  if(mistAnimFrame%3!==0)return;
  const t=performance.now()/1000;
  const activeRadius2=28*28;

  for(const patch of mistPatches){
    if(!patch || patch.metadata?.cleared)continue;

    const bx=patch.metadata.baseX;
    const bz=patch.metadata.baseZ;
    const dx=bx-player.position.x;
    const dz=bz-player.position.z;
    const dist2=dx*dx+dz*dz;

    // Lo muy lejano permanece estático: visualmente sigue cubriendo el mapa sin consumir CPU.
    if(dist2>activeRadius2)continue;

    const phase=patch.metadata.phase||0;
    patch.position.x=bx+Math.sin(t*.16+phase)*.18;
    patch.position.z=bz+Math.cos(t*.12+phase)*.14;
    patch.position.y=Math.sin(t*.18+phase)*.04;

    for(const layer of patch.getChildMeshes(false)){
      const lm=layer.metadata||{};
      layer.position.y=(lm.baseY||.8)+Math.sin(t*.36+(lm.phase||0))*.08;
    }
  }
}
...
playerHighlight.blurHorizontalSize=0.12;
playerHighlight.blurVerticalSize=0.12;
...
function updateDarkness(){
  ...
  // Oscurece el mapa no recorrido, pero deja suficiente lectura local.
  scene.fogStart=18.0;
  scene.fogEnd=B.Scalar.Clamp(48.0-low*6.5,38.0,48.0);

  mistMat.alpha=B.Scalar.Lerp(.80,.92,low);
  cloudDark.alpha=B.Scalar.Lerp(.44,.58,low);
  cloudMid.alpha=B.Scalar.Lerp(.34,.46,low);
  cloudLight.alpha=B.Scalar.Lerp(.24,.34,low);

  stormEmitter.position.set(player.position.x,player.position.y,player.position.z);

  if(performance.now()>nextNaturalLightning){
    nextNaturalLightning=performance.now()+5800+rand()*7200;
    flash('#eef7ff',70,.60);
    setTimeout(()=>flash('#c9deed',55,.28),100);
    if(audioCtx){
      setTimeout(()=>{tone(55,.20,'triangle',.022);},460+rand()*780);
    }
  }
}
...
async function loadCharacter(){
  try{
    ...
    meshes.forEach(m=>{
      ...
      try{ playerHighlight.addMesh(m,new B.Color3(.72,.97,1.0)); }catch(_){}
      if(m.material){
        try{
          const em = m.material.emissiveColor || new B.Color3(0,0,0);
          m.material.emissiveColor = em.add(new B.Color3(.08,.11,.15));
        }catch(_){}
      }
    });

    // Aro/base luminosa discreta para ubicar al personaje en las tinieblas.
    const playerMarkerMat=new B.StandardMaterial('playerMarkerMat',scene);
    playerMarkerMat.diffuseColor=B.Color3.FromHexString('#8de8ff');
    playerMarkerMat.emissiveColor=B.Color3.FromHexString('#9ef1ff');
    playerMarkerMat.alpha=.55;
    playerMarkerMat.specularColor=new B.Color3(0,0,0);
    const playerMarker=B.MeshBuilder.CreateDisc('playerMarker',{radius:1.02,tessellation:44,sideOrientation:B.Mesh.DOUBLESIDE},scene);
    playerMarker.parent=player;
    playerMarker.position.set(0,.05,0);
    playerMarker.rotation.x=Math.PI/2;
    playerMarker.material=playerMarkerMat;
    playerMarker.isPickable=false;

    const beaconMat=new B.StandardMaterial('playerBeaconMat',scene);
    beaconMat.diffuseColor=B.Color3.FromHexString('#aaf3ff');
    beaconMat.emissiveColor=B.Color3.FromHexString('#d7fbff');
    beaconMat.alpha=.55;
    beaconMat.specularColor=new B.Color3(0,0,0);
    const playerBeacon=B.MeshBuilder.CreateCylinder('playerBeacon',{height:1.2,diameterTop:0.0,diameterBottom:.42,tessellation:20},scene);
    playerBeacon.parent=player;
    playerBeacon.position.set(0,2.25,0);
    playerBeacon.rotation.x=Math.PI;
    playerBeacon.material=beaconMat;
    playerBeacon.isPickable=false;
    ...
    setStatus(`ZOOM + NIEBLA v23 · ${groundBoneMode} · ${boneNames}`);
