...
// ---------------------------------------------------------
// DESCUBRIMIENTO REAL DEL MAPA
// ---------------------------------------------------------
const clearedTrail=[];
let lastTrailPoint=null;
let mistAnimFrame=0;

function layerWorldCenter(layer){
  const root=layer.metadata?.patchRoot;
  if(!root)return {x:0,z:0};
  return {
    x:(root.metadata?.baseX||0)+(layer.metadata?.localX||0),
    z:(root.metadata?.baseZ||0)+(layer.metadata?.localZ||0)
  };
}

function clearMistLayersNear(x,z,radius=PLAYER_CLEAR_RADIUS){
  for(const layer of mistLayers){
    if(!layer || layer.isDisposed?.() || layer.metadata?.cleared)continue;

    const p=layerWorldCenter(layer);
    const dx=p.x-x;
    const dz=p.z-z;
    const limit=radius+(layer.metadata?.visualRadius||2.5);

    if(dx*dx+dz*dz < limit*limit){
      layer.metadata.cleared=true;
  ...
  for(const patch of mistPatches){
    ...
    const visible=patch.getChildMeshes(false).some(
      child=>child.isEnabled() && !child.metadata?.cleared
    );
    if(!visible){
...
function recordClearedTrail(){
  ...
  if(!lastTrailPoint || B.Vector3.DistanceSquared(p,lastTrailPoint)>.32*.32){
    clearedTrail.push({x:p.x,z:p.z});
    if(clearedTrail.length>650)clearedTrail.shift();
    lastTrailPoint=p.clone();
    clearMistLayersNear(p.x,p.z,PLAYER_CLEAR_RADIUS);
  }
}
...
function puffInsideClearedTrail(wx,wz){
  for(let i=clearedTrail.length-1;i>=0;i--){
    ...
    if(dx*dx+dz*dz<PLAYER_CLEAR_RADIUS*PLAYER_CLEAR_RADIUS)return true;
  }
  return false;
}
...
function updateCloudClearing(){
  ...
  // Garantía: no puede existir una capa de tiniebla dentro del
  // área limpia inmediata del Bebé.
  clearMistLayersNear(
    player.position.x,
    player.position.z,
    PLAYER_CLEAR_RADIUS
  ...
  for(const patch of mistPatches){
    ...
    patch.position.x=Math.sin(t*.16+phase)*.15;
    patch.position.z=Math.cos(t*.12+phase)*.12;
    patch.position.y=Math.sin(t*.18+phase)*.035;

    for(const layer of patch.getChildMeshes(false)){
      if(layer.metadata?.cleared)continue;
      const lm=layer.metadata||{};
      layer.position.y=(lm.baseY||.8)+Math.sin(t*.36+(lm.phase||0))*.07;
    }
  }
}
...
playerHighlight.blurHorizontalSize=0.07;
playerHighlight.blurVerticalSize=0.07;
...
function updateDarkness(){
  ...
  // El fog general empieza lejos: no pone una película gris
  // encima del personaje ni del círculo descubierto.
  scene.fogStart=30.0;
  scene.fogEnd=B.Scalar.Clamp(66.0-low*8.0,54.0,66.0);

  // Fuera del recorrido, las tinieblas físicas siguen densas.
  mistMat.alpha=B.Scalar.Lerp(.82,.94,low);
  cloudDark.alpha=B.Scalar.Lerp(.40,.54,low);
  cloudMid.alpha=B.Scalar.Lerp(.30,.42,low);
  cloudLight.alpha=B.Scalar.Lerp(.20,.30,low);
...
async function loadCharacter(){
  try{
    ...
    meshes.forEach(m=>{
      ...
      try{
        playerHighlight.addMesh(m,new B.Color3(.30,.78,1.0));
      }catch(_){}
    });
    ...
    playerMarkerMat.alpha=.18;
    playerMarkerMat.specularColor=new B.Color3(0,0,0);
    const playerMarker=B.MeshBuilder.CreateDisc('playerMarker',{radius:.68,tessellation:36,sideOrientation:B.Mesh.DOUBLESIDE},scene);
    ...
    setStatus(`DESCUBRIMIENTO v24 · ${groundBoneMode} · ${boneNames}`);
