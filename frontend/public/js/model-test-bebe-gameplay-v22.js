...
let mistId=0;
for(let z=-55;z<=22;z+=5.4){
  for(let x=-27;x<=27;x+=5.4){
    if(rand()<.05)continue;
...
function clearMistNear(x,z,radius=4.55){
...
function puffInsideClearedTrail(wx,wz){
  ...
  for(let i=clearedTrail.length-1;i>=0;i--){
    ...
    if(dx*dx+dz*dz<4.55*4.55)return true;
  }
  return false;
}
...
const camera=new B.FreeCamera('camera',new B.Vector3(0,30,8.5),scene);
camera.inputs.clear();
camera.fov=1.00;
camera.minZ=.08;
camera.maxZ=220;
...
playerHighlight.blurHorizontalSize=0.18;
playerHighlight.blurVerticalSize=0.18;
...
function updateDarkness(){
  ...
  scene.fogStart=22.0;
  scene.fogEnd=B.Scalar.Clamp(55.0-low*7.0,44.0,55.0);

  mistMat.alpha=B.Scalar.Lerp(.70,.84,low);
  cloudDark.alpha=B.Scalar.Lerp(.38,.52,low);
  cloudMid.alpha=B.Scalar.Lerp(.28,.40,low);
...
async function loadCharacter(){
  try{
    ...
    meshes.forEach(m=>{
      ...
      try{ playerHighlight.addMesh(m,new B.Color3(.50,.92,1.0)); }catch(_){}
      if(m.material){
        try{
          const em = m.material.emissiveColor || new B.Color3(0,0,0);
          m.material.emissiveColor = em.add(new B.Color3(.04,.07,.10));
        }catch(_){}
      }
    });
    ...
    playerMarkerMat.alpha=.34;
    playerMarkerMat.specularColor=new B.Color3(0,0,0);
    const playerMarker=B.MeshBuilder.CreateDisc('playerMarker',{radius:.72,tessellation:36,sideOrientation:B.Mesh.DOUBLESIDE},scene);
    playerMarker.parent=player;
    playerMarker.position.set(0,.04,0);
    ...
    setStatus(`RENDIMIENTO v22 · ${groundBoneMode} · ${boneNames}`);
...
const baseCamX=player.position.x*.40;
const baseCamY=30.0;
const baseCamZ=player.position.z+8.4;
const lookZ=player.position.z-6.1;
