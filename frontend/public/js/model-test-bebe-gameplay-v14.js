...
// Materiales de tormenta: masas opacas/semitransparentes, sin contornos circulares.
const cloudDark=new B.StandardMaterial('cloudDark',scene);
cloudDark.diffuseColor=B.Color3.FromHexString('#26313a');
cloudDark.emissiveColor=B.Color3.FromHexString('#111820');
cloudDark.alpha=.72; cloudDark.backFaceCulling=false;

const cloudMid=new B.StandardMaterial('cloudMid',scene);
cloudMid.diffuseColor=B.Color3.FromHexString('#3a4853');
cloudMid.emissiveColor=B.Color3.FromHexString('#18232c');
cloudMid.alpha=.63; cloudMid.backFaceCulling=false;

const cloudLight=new B.StandardMaterial('cloudLight',scene);
cloudLight.diffuseColor=B.Color3.FromHexString('#64717a');
cloudLight.emissiveColor=B.Color3.FromHexString('#222c33');
cloudLight.alpha=.42; cloudLight.backFaceCulling=false;
...
// Separadores visuales de cuadras: pasos peatonales cartoon.
// Ayudan a leer la profundidad como dos cuadras completas.
function crosswalk(z){
  for(let i=0;i<5;i++){
    const stripe=box('Crosswalk',5.25,.025,.24,0,.105,z-1.0+i*.48,mats.sidewalk);
    stripe.material.alpha=.86;
  }
}
crosswalk(-1.4);
crosswalk(-11.8);
...
function cloudPuff(root,x,y,z,sx,sy,sz,material){
  const p=B.MeshBuilder.CreateIcoSphere('StormPuff',{radius:1,subdivisions:2,flat:false},scene);
  p.parent=root;p.position.set(x,y,z);p.scaling.set(sx,sy,sz);p.material=material;
  p.isPickable=false;p.receiveShadows=false;
  p.visibility=1;
  p.metadata={
...
function stormBank(z,width=9,height=4.0,depth=3.6,density=22){
  ...
  for(let i=0;i<density;i++){
    ...
    const base=.70+rand()*1.0;
    const material=i%5===0?cloudLight:(i%2?cloudDark:cloudMid);
    cloudPuff(root,x,y,zz,base*(1.2+rand()*.8),base*(.48+rand()*.32),base*(.8+rand()*.7),material);
  }
...
function stormCeiling(z){
  ...
  for(let i=0;i<13;i++){
    const x=(rand()-.5)*12;
    const zz=(rand()-.5)*8;
    const base=1.3+rand()*1.1;
    cloudPuff(root,x,4.7+rand()*.8,zz,base*1.8,base*.55,base*1.35,i%3?cloudDark:cloudMid);
  }
  darknessClouds.push(root);
}
...
rain.minEmitBox=new B.Vector3(-7,4,-7);
rain.maxEmitBox=new B.Vector3(7,6,7);
...
const GUIDE_STEPS=[
  {z:12.5,id:'voz',label:'VOZ SUAVE',text:'Detente un instante. Sientes que debes continuar hacia adelante, sin desviarte.',mode:'voice'},
  {z:9.2,id:'luces',label:'LUZ',text:'Una luz breve aparece delante de ti. Síguela.',mode:'light'},
  {z:5.7,id:'truenos',label:'TRUENOS',text:'El trueno te advierte que no sigas por el camino lateral. Continúa por la vía principal.',mode:'thunder'},
  {z:2.2,id:'relampagos',label:'RELÁMPAGO',text:'El relámpago ilumina por un instante el siguiente tramo del camino.',mode:'lightning'},
  {z:-1.4,id:'terremoto',label:'TERREMOTO',text:'El suelo tiembla. El camino firme sigue delante de ti; no te apartes.',mode:'quake'},
  {z:-5.0,id:'hambre',label:'HAMBRE',text:'Sientes necesidad y recuerdas lo verdaderamente importante. Continúa hacia el lugar seguro.',mode:'hunger'},
  {z:-8.9,id:'honra',label:'HONRA',text:'Has mantenido tu compromiso. Sigue adelante aunque el camino sea difícil.',mode:'honor'},
  {z:-12.8,id:'gloria',label:'GLORIA',text:'Una claridad mayor aparece delante. El destino está cerca.',mode:'glory'},
  {z:-16.4,id:'trompeta',label:'TROMPETA',text:'El llamado final te invita a reunirte y llegar al destino.',mode:'trumpet'}
];
...
function revealNextPath(duration=4.5){
  ...
  const obs=scene.onBeforeRenderObservable.add(()=>{
    ...
    darknessClouds.forEach(root=>{
      const hz=root.metadata?.homeZ ?? root.position.z;
      const dz=hz-player.position.z;
      // Limpia principalmente nubes en la zona actual y 6m por delante.
      if(dz<2.5 && dz>-8.0){
        root.metadata.cleared=Math.max(root.metadata.cleared||0,progress);
      }
    });
    if(elapsed>duration){
...
function triggerGuide(step){
  ...
  if(step.mode==='voice'){
    ...
    createGuidingLights(player.position.z-.8,-19.0,8);
...
function updateGuidedPath(){
  ...
  pathProgress=B.Scalar.Clamp(((14-player.position.z)/(14-(-20)))*100,0,100);
  ...
  // Destino alcanzado.
  if(!reachedDestination && player.position.z<=-18.8 && Math.abs(player.position.x)<2.2){
...
function puffInsideClearedTrail(wx,wz){
  ...
  for(let i=clearedTrail.length-1;i>=0;i--){
    ...
    if(dx*dx+dz*dz<3.15*3.15)return true;
  }
  return false;
}
...
function updateCloudClearing(){
  ...
  const guideActive=(performance.now()-lastGuideTime)<5000;
  ...
  for(const puff of stormPuffs){
    ...
    // En nuestro mapa avanzar significa ir hacia Z negativo.
    // Zona inmediata: alrededor del Bebé.
    const aroundPlayer=(dx*dx+dz*dz)<4.0*4.0;

    // Corredor delante: aproximadamente 12 metros.
    // Se hace un poco más ancho cuando acaba de recibirse una guía.
    const aheadDistance=-dz; // positivo si la nube está delante.
    const corridorWidth=guideActive?4.7:3.7;
    const aheadCorridor=
      aheadDistance>=-1.5 &&
      aheadDistance<=12.5 &&
      Math.abs(dx)<corridorWidth;

    // El camino ya recorrido se queda abierto.
    const oldTrail=puffInsideClearedTrail(wp.x,wp.z);

    let target=1;

    if(puff.metadata?.ceiling){
      // El cielo sigue tormentoso, pero el techo se aclara un poco sobre el corredor.
      if(aroundPlayer || aheadCorridor || oldTrail)target=.36;
      else target=.88;
    }else{
      // Nubes bajas: se quitan realmente del camino.
      if(oldTrail)target=.015;
      else if(aroundPlayer)target=.010;
      else if(aheadCorridor)target=guideActive?.025:.08;
      else target=.96;
    }

    puff.metadata.targetVisibility=target;

    // Se abre rápido; si está fuera del corredor se recupera lentamente,
    // salvo si forma parte de la huella permanente.
    const opening=target<puff.visibility;
    const rate=opening?.16:.022;
    puff.visibility=B.Scalar.Lerp(puff.visibility,target,rate);
  }
}
...
const camera=new B.FreeCamera('camera',new B.Vector3(0,7.6,13.0),scene);
camera.inputs.clear();
camera.fov=.78;
camera.minZ=.08;
camera.maxZ=180;
...
function updateDarkness(){
  ...
  const guidedOpen=(performance.now()-lastGuideTime<5000)?1:0;

  // La cámara ahora muestra mucha más distancia.
  // La tormenta sigue existiendo, pero el corredor despejado puede verse a dos cuadras.
  scene.fogStart=B.Scalar.Lerp(6.5,7.8,guidedOpen);
  scene.fogEnd=B.Scalar.Clamp(
    B.Scalar.Lerp(27.0,34.0,guidedOpen)-low*7.0,
    19.0,
    34.0
  );

  cloudDark.alpha=B.Scalar.Lerp(.62,.80,low);
  cloudMid.alpha=B.Scalar.Lerp(.52,.72,low);
  cloudLight.alpha=B.Scalar.Lerp(.30,.48,low);

  // Solo movimiento atmosférico suave de los bancos completos.
  // La desaparición local se controla puff por puff en updateCloudClearing().
  const t=performance.now()/1000;
  darknessClouds.forEach((c,i)=>{
    c.position.x=Math.sin(t*.085+i*.72)*.13;
  });
...
async function loadCharacter(){
  try{
    ...
    setStatus(`CORREDOR v14 · ${groundBoneMode} · ${boneNames}`);
...
scene.onBeforeRenderObservable.add(()=>{
  ...
  // Cámara más alta y retirada: permite ver aproximadamente dos cuadras
  // mientras mantiene al Bebé en la parte baja-central de la pantalla.
  const baseCamX=player.position.x;
  const baseCamY=7.55;
  const baseCamZ=player.position.z+12.8;
  const lookZ=player.position.z-4.4;
  ...
  camera.setTarget(new B.Vector3(player.position.x,.80,lookZ));
});
