...
function triggerGuide(step){
  ...
  // Todas las guías abren parcialmente el camino.
  revealNextPath(4.8);
...
function updateCloudClearing(){
  // Zona limpia móvil alrededor del personaje y, más importante, delante de él.
  darknessClouds.forEach(root=>{
    const hz=root.metadata?.homeZ ?? root.position.z;
    const dz=hz-player.position.z;

    // 1.0 = despejada. Se despeja si ya pasamos por ahí o si está cerca por delante.
    let desired=0;
    if(dz>1.0) desired=.72;                  // nubes que quedaron detrás
    if(dz<=1.0 && dz>=-4.5) desired=.80;     // corredor inmediato delante
    if(dz<-4.5) desired=Math.max(0,(root.metadata?.cleared||0)*.72);

    // Si hubo una guía reciente, abre más el corredor por delante.
    if(performance.now()-lastGuideTime<4800 && dz<=1.5 && dz>=-7.0){
      desired=Math.max(desired,.92);
    }

    const current=root.metadata.cleared||0;
    root.metadata.cleared=B.Scalar.Lerp(current,desired,.035);

    // Reducimos altura y densidad, pero no hacemos desaparecer todo el cielo.
    const clear=root.metadata.cleared;
    const s=B.Scalar.Lerp(1.0,.28,clear);
    root.scaling.y=B.Scalar.Lerp(root.scaling.y,.42+.58*s,.06);
    root.scaling.x=B.Scalar.Lerp(root.scaling.x,.72+.28*s,.06);
    root.scaling.z=B.Scalar.Lerp(root.scaling.z,.72+.28*s,.06);

    // Levanta las nubes que se están abriendo para crear un corredor visual.
    root.position.y=B.Scalar.Lerp(root.position.y,clear*2.0,.045);
  });
}
...
function updateDarkness(){
  ...
  // La oscuridad sigue siendo fuerte, pero el corredor se abre con el progreso.
  const guidedOpen=(performance.now()-lastGuideTime<4800)?1:0;
  scene.fogStart=B.Scalar.Lerp(2.1,3.4,guidedOpen);
  scene.fogEnd=B.Scalar.Lerp(9.0,12.8,guidedOpen)-low*2.5;

  cloudDark.alpha=B.Scalar.Lerp(.66,.84,low);
  cloudMid.alpha=B.Scalar.Lerp(.56,.76,low);
  cloudLight.alpha=B.Scalar.Lerp(.34,.53,low);

  const t=performance.now()/1000;
  darknessClouds.forEach((c,i)=>{
    // Mantiene el desplazamiento atmosférico, preservando la elevación de despeje.
    const clear=c.metadata?.cleared||0;
    c.position.x=Math.sin(t*.10+i*.7)*.16;
    c.position.y=B.Scalar.Lerp(c.position.y,clear*2.0+Math.sin(t*.13+i)*.05,.06);
  });
...
async function loadCharacter(){
  try{
    ...
    setStatus(`CAMINO GUIADO v13 · ${groundBoneMode} · ${boneNames}`);
