...
box('worldBase',34,.10,50,0,-.18,0,mats.stormGround);
...
const CROSS_STREETS=[17,9,1,-7,-15];
CROSS_STREETS.forEach((z,i)=>{
  box('crossRoad'+i,28,.12,2.65,0,-.105,z,mats.road);

  // Cruce peatonal en ambos lados de la intersección.
  for(let s=-2;s<=2;s++){
    const stripe=box('crossStripe'+i+'_'+s,.42,.025,2.05,s*.78,.075,z,mats.sidewalk);
...
for(let z=-22;z<=22;z+=2.25){
  for(let x=-11.5;x<=11.5;x+=2.25){
...
rain.minEmitBox=new B.Vector3(-17,5,-20);
rain.maxEmitBox=new B.Vector3(17,8,20);
...
function recordClearedTrail(){
  ...
  if(!lastTrailPoint || B.Vector3.DistanceSquared(p,lastTrailPoint)>.55*.55){
    ...
    if(clearedTrail.length>500)clearedTrail.shift();
  }
}
...
function puffInsideClearedTrail(wx,wz){
  ...
  for(let i=clearedTrail.length-1;i>=0;i--){
    ...
    if(dx*dx+dz*dz<3.55*3.55)return true;
  }
  return false;
}
...
function updateCloudClearing(){
  ...
  for(const puff of stormPuffs){
    ...
    // El círculo inmediato se abre para que el jugador vea dónde pisa.
    const aroundPlayer=(dx*dx+dz*dz)<3.9*3.9;

    // Todo lo que ya recorrió queda despejado permanentemente.
    const oldTrail=puffInsideClearedTrail(wp.x,wp.z);

    let target=1;

    if(puff.metadata?.ceiling){
      // El techo de tormenta permanece para conservar la atmósfera.
      // Solo se aclara un poco exactamente encima del recorrido.
      target=(aroundPlayer||oldTrail)?.34:.88;
    }else{
      // Nubes bajas: SOLO desaparecen donde se caminó.
      // No abrimos automáticamente cuadras futuras.
      if(oldTrail)target=.004;
      else if(aroundPlayer)target=.008;
      else target=.97;
    }

    puff.metadata.targetVisibility=target;

    // Abrir es rápido; las nubes fuera de la huella recuperan densidad.
    // Las nubes de la huella nunca se recuperan porque oldTrail seguirá siendo true.
    const opening=target<puff.visibility;
    const rate=opening?.22:.018;
...
const camera=new B.FreeCamera('camera',new B.Vector3(0,27,14),scene);
camera.inputs.clear();
camera.fov=1.00;
camera.minZ=.08;
camera.maxZ=220;
...
function updateDarkness(){
  ...
  // La cámara puede ver muchas cuadras.
  // La ocultación principal la hacen las nubes físicas, no una niebla demasiado corta.
  scene.fogStart=18.0;
  scene.fogEnd=B.Scalar.Clamp(48.0-low*7.0,36.0,48.0);

  cloudDark.alpha=B.Scalar.Lerp(.66,.82,low);
  cloudMid.alpha=B.Scalar.Lerp(.56,.75,low);
  cloudLight.alpha=B.Scalar.Lerp(.34,.50,low);

  const t=performance.now()/1000;
  darknessClouds.forEach((c,i)=>{
    c.position.x=Math.sin(t*.075+i*.72)*.12;
  });
...
async function loadCharacter(){
  try{
    ...
    setStatus(`VISTA AMPLIA v15 · ${groundBoneMode} · ${boneNames}`);
...
scene.onBeforeRenderObservable.add(()=>{
  ...
  // Vista alta y casi cenital tipo juego táctico:
// el Bebé queda pequeño y se alcanzan a percibir varias cuadras.
const baseCamX=player.position.x;
const baseCamY=27.0;
const baseCamZ=player.position.z+8.2;
const lookZ=player.position.z-4.2;

if(quakeTimer>0){
  quakeTimer-=dt;
  const q=.12*Math.min(1,quakeTimer);
  camera.position.set(
...
camera.setTarget(new B.Vector3(player.position.x,.55,lookZ));
});
