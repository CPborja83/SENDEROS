...
function shrubCluster(x,z,s=1){
  const a=sphere('shrubA',.82*s,x,.34,z,mats.leaves2);
  const b=sphere('shrubB',.64*s,x-.26*s,.30,z+.18*s,mats.leaves);
  const c=sphere('shrubC',.58*s,x+.24*s,.28,z-.16*s,mats.leaves3);
  [a,b,c].forEach(o=>{o.scaling.y*=.72;shadows.addShadowCaster(o);});
}

function addFrontYard(shell,depth=1.35,count=5){
  const x=shell.x, z=shell.z, w=shell.w, d=shell.d, dir=shell.towardRoad;
  const yardX=x+dir*(w/2+depth/2-.05);
  const strip=box(shell.name+'FrontYard',depth,.05,Math.max(2.2,d-.85),yardX,.03,z,mats.grass);
  strip.receiveShadows=true;
  for(let i=0;i<count;i++){
    const frac=(i/(Math.max(1,count-1))-.5);
    const sz=.72+Math.random()*.24;
    shrubCluster(yardX+dir*(Math.random()*.18-.09),z+frac*(d-1.7)+(Math.random()-.5)*.25,sz);
  }
}
...
CROSS_STREETS.forEach((z,i)=>{
  box('crossRoad'+i,50,.12,1.95,0,-.105,z,mats.road);

  // Cruce peatonal en ambos lados de la intersección.
  for(let s=-2;s<=2;s++){
    const stripe=box('crossStripe'+i+'_'+s,.30,.025,1.40,s*.52,.075,z,mats.sidewalk);
...
makeHouse(-10.4,12.0,'CASA FAMILIAR',mats.wall1);
makeHouse( 10.4,11.0,'CASA FAMILIAR',mats.wall3);

// ---------------------------------------------------------
// CANCHA DE FÚTBOL SINTÉTICA
// ---------------------------------------------------------
function makeSoccerField(){
  const x=-17.2,z=-.3,w=11.5,d=7.2;
...
building(-10.8,-39.5,5.0,5.2,1.5,mats.wall2,true);
building( 10.8,-45.0,5.1,5.1,1.5,mats.wall3,true);
...
[-17,-8,1,10,18].forEach((z,i)=>{lamp(-2.35,z);if(i%2===0)lamp(2.35,z+2.2)});
...
function makeMistPatch(x,z,id){
  ...
  for(let i=0;i<7;i++){
    ...
    plane.parent=root;
    plane.position.set((rand()-.5)*1.8,.18+rand()*.36,(rand()-.5)*1.8);
    plane.rotation.x=Math.PI/2;
    plane.rotation.z=rand()*Math.PI*2;
    plane.material=mistMat;
    plane.isPickable=false;
    plane.visibility=.86+rand()*.14;
    plane.metadata={
      baseY:plane.position.y,
      rot:plane.rotation.z,
      phase:rand()*Math.PI*2,
      baseScale:.95+rand()*.38
    };
...
// Tinieblas densas repartidas por casi todo el mapa.
let mistId=0;
for(let z=-56;z<=23;z+=4.0){
  for(let x=-29;x<=29;x+=4.0){
    if(rand()<.02)continue;
    makeMistPatch(x+(rand()-.5)*.75,z+(rand()-.5)*.75,mistId++);
  }
}
...
function triggerGuide(step){
  ...
  if(step.mode==='voice'){
    ...
    tone(64,.32,'triangle',.03);setTimeout(()=>tone(56,.22,'sine',.025),70);
    setTimeout(()=>speak(step.text,{pitch:.84,rate:.90}),60);
  }
...
function puffInsideClearedTrail(wx,wz){
  for(const p of clearedTrail){
    const dx=wx-p.x;
    const dz=wz-p.z;
    if(dx*dx+dz*dz<3.35*3.35)return true;
  }
  return false;
}
...
function updateCloudClearing(){
  ...
  for(const patch of mistPatches){
    ...
    const aroundPlayer=(dx*dx+dz*dz)<3.0*3.0;
    const oldTrail=puffInsideClearedTrail(bx,bz);

    if(aroundPlayer || oldTrail){
    ...
    patch.position.x=bx+Math.sin(t*.15+phase)*.18;
    patch.position.z=bz+Math.cos(t*.12+phase)*.14;

    for(const layer of patch.getChildMeshes(false)){
      const lm=layer.metadata||{};
      layer.position.y=(lm.baseY||.25)+Math.sin(t*.38+(lm.phase||0))*.08;
      layer.rotation.z=(lm.rot||0)+Math.sin(t*.10+(lm.phase||0))*.03;
      const pulse=(lm.baseScale||1)*(1+Math.sin(t*.22+(lm.phase||0))*.045);
      layer.scaling.x=B.Scalar.Lerp(layer.scaling.x,pulse,.07);
      layer.scaling.y=B.Scalar.Lerp(layer.scaling.y,pulse*.96,.07);
    }
  }
}
...
const camera=new B.FreeCamera('camera',new B.Vector3(0,27,7.5),scene);
camera.inputs.clear();
camera.fov=.96;
camera.minZ=.08;
camera.maxZ=220;
...
function triggerVoiceEvent(id){
  ...
  else if(id==='truenos'){tone(64,.32,'triangle',.03);setTimeout(()=>tone(56,.22,'sine',.025),70);flash('#cbd6df',60,.18);speak(text,{pitch:.82,rate:.92});}
...
function updateDarkness(){
  ...
  // La niebla general ahora es más cerrada; solo el camino despejado se lee con claridad.
  scene.fogStart=16.0;
  scene.fogEnd=B.Scalar.Clamp(46.0-low*6.0,34.0,46.0);

  mistMat.alpha=B.Scalar.Lerp(.84,.96,low);
  cloudDark.alpha=B.Scalar.Lerp(.34,.46,low);
  cloudMid.alpha=B.Scalar.Lerp(.26,.38,low);
  cloudLight.alpha=B.Scalar.Lerp(.18,.28,low);
...
async function loadCharacter(){
  try{
    ...
    setStatus(`DETALLE + NIEBLA v20 · ${groundBoneMode} · ${boneNames}`);
...
scene.onBeforeRenderObservable.add(()=>{
  ...
  let nextX=B.Scalar.Clamp(player.position.x+velocity.x*dt,-27.0,27.0);
  ...
  // Vista más cercana para apreciar detalles, sin perder lectura general del mapa.
const baseCamX=player.position.x*.46;
const baseCamY=27.0;
const baseCamZ=player.position.z+7.4;
const lookZ=player.position.z-5.2;
