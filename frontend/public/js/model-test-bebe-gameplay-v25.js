...
function cartoonize(mesh,width=1.25,alpha=.58){
  ...
  const important=/(room|hospital|church|library|dentist|house|wall|back|front|side|bed|desk|table|counter|arcade|pew|pulpit|shelf|cabinet|monitor|screen|court|goal|basket|soccer|destination|door|awning|building)/.test(n);
  if(important){
    try{
...
function makeDentist(){
  ...
  wallZ('DentistDivider',x,z,d-.3,mats.wall3,z+.4,1.1);
  simpleDesk(x+2.2,z+2.5);

  // Sillón dental
  const chair=box('DentalChair',.85,.42,1.75,x-2.0,.38,z-.3,mats.dental);
  chair.rotation.x=-.10;
  const head=sphere('DentalHead',.62,x-2.0,.74,z-.92,mats.dental);
  const lampPole=cyl('DentalLampPole',.10,1.55,x-.75,.78,z-.65,mats.metal);
  const lamp=sphere('DentalLamp',.38,x-1.0,1.45,z-.65,mats.lamp);
  [chair,head,lampPole,lamp].forEach(o=>shadows.addShadowCaster(o));
  addCollider('sillón dental',x-2.0,z-.3,1.05,1.9,.04);

  const cabinet=box('DentalCabinet',1.7,.82,.52,x-2.4,.41,z+2.6,mats.counter);
  shadows.addShadowCaster(cabinet);addCollider('gabinete dental',x-2.4,z+2.6,1.8,.62,.03);

  registerActivity('dentist','LIMPIEZA DENTAL',x-1.4,z-.4,1.5);
  registerDistraction('Pantalla clínica',x+2.2,z+2.5,1.25,.60);
  makeGroundLabel('RECEPCIÓN',x+2.1,z+3.1,2.1,.38,'#4b7779','#ffffff',23);
  makeGroundLabel('CONSULTORIO',x-2.0,z-3.1,2.4,.38,'#4b7779','#ffffff',23);
}
makeDentist();
...
// Tormenta superior: permanece sobre el mapa, pero no sustituye a las tinieblas del suelo.
stormCeiling(12);stormCeiling(-12);stormCeiling(-36);
stormBank(8,18,3.0,6.0,18);
stormBank(-20,18,3.0,6.0,18);
stormBank(-45,18,3.0,6.0,18);
...
let mistId=0;
// v25: los parches bajos quedan desactivados.
// Las tinieblas ahora se representan con una capa continua sobre TODO el mapa.

// ---------------------------------------------------------
// TINIEBLAS CONTINUAS SOBRE TODO EL MAPA
// ---------------------------------------------------------
// Esta capa está físicamente encima de calles, jardines y edificios.
// Solo se vuelve transparente donde realmente camina el personaje.
const FOG_WORLD_MIN_X=-31;
const FOG_WORLD_MAX_X=31;
const FOG_WORLD_MIN_Z=-59;
const FOG_WORLD_MAX_Z=23;
const FOG_WORLD_W=FOG_WORLD_MAX_X-FOG_WORLD_MIN_X;
const FOG_WORLD_D=FOG_WORLD_MAX_Z-FOG_WORLD_MIN_Z;

const FOG_TEX_W=1024;
const FOG_TEX_H=1354;
const FOG_CLEAR_RADIUS=3.05;

// Textura visible de vapor/tinieblas.
const fogSmokeTex=new B.DynamicTexture(
  'FogSmokeTexture',
  {width:512,height:512},
  scene,
  false
);
fogSmokeTex.hasAlpha=true;
fogSmokeTex.wrapU=B.Texture.WRAP_ADDRESSMODE;
fogSmokeTex.wrapV=B.Texture.WRAP_ADDRESSMODE;

const fsctx=fogSmokeTex.getContext();
fsctx.clearRect(0,0,512,512);
fsctx.fillStyle='rgba(13,20,29,1)';
fsctx.fillRect(0,0,512,512);

// Muchas masas suaves superpuestas para que parezca vapor y no manchas sólidas.
let fogSeed=918273;
function fogRand(){
  fogSeed=(fogSeed*1664525+1013904223)>>>0;
  return fogSeed/4294967296;
}
for(let i=0;i<115;i++){
  const x=fogRand()*512;
  const y=fogRand()*512;
  const r=28+fogRand()*105;
  const alpha=.13+fogRand()*.27;
  const g=fsctx.createRadialGradient(x,y,2,x,y,r);
  g.addColorStop(0,`rgba(90,107,120,${alpha+.12})`);
  g.addColorStop(.35,`rgba(55,69,82,${alpha})`);
  g.addColorStop(.72,`rgba(28,38,49,${alpha*.70})`);
  g.addColorStop(1,'rgba(8,14,22,0)');
  fsctx.fillStyle=g;
  fsctx.fillRect(x-r,y-r,r*2,r*2);
}
fogSmokeTex.update(false);

// Más repeticiones = textura fina de vapor, no un solo círculo/mancha.
fogSmokeTex.uScale=2.15;
fogSmokeTex.vScale=2.65;

// Máscara dinámica: blanco/alpha = oculto; transparente = descubierto.
const fogMaskTex=new B.DynamicTexture(
  'FogDiscoveryMask',
  {width:FOG_TEX_W,height:FOG_TEX_H},
  scene,
  false
);
fogMaskTex.hasAlpha=true;
fogMaskTex.wrapU=B.Texture.CLAMP_ADDRESSMODE;
fogMaskTex.wrapV=B.Texture.CLAMP_ADDRESSMODE;

const fmctx=fogMaskTex.getContext();
fmctx.clearRect(0,0,FOG_TEX_W,FOG_TEX_H);
fmctx.fillStyle='rgba(255,255,255,.975)';
fmctx.fillRect(0,0,FOG_TEX_W,FOG_TEX_H);
fogMaskTex.update(false);

const fogCoverMat=new B.StandardMaterial('FogCoverMaterial',scene);
fogCoverMat.diffuseTexture=fogSmokeTex;
fogCoverMat.emissiveTexture=fogSmokeTex;
fogCoverMat.opacityTexture=fogMaskTex;
fogCoverMat.diffuseColor=B.Color3.FromHexString('#263440');
fogCoverMat.emissiveColor=B.Color3.FromHexString('#101820');
fogCoverMat.specularColor=new B.Color3(0,0,0);
fogCoverMat.alpha=.985;
fogCoverMat.backFaceCulling=false;
fogCoverMat.disableLighting=true;

// CreateGround da una superficie horizontal que cubre el mapa entero.
const fogCover=B.MeshBuilder.CreateGround(
  'FogOfWarCover',
  {width:FOG_WORLD_W,height:FOG_WORLD_D,subdivisions:1},
  scene
);
fogCover.position.set(
  (FOG_WORLD_MIN_X+FOG_WORLD_MAX_X)/2,
  4.25,
  (FOG_WORLD_MIN_Z+FOG_WORLD_MAX_Z)/2
);
fogCover.material=fogCoverMat;
fogCover.isPickable=false;
fogCover.receiveShadows=false;
fogCover.renderingGroupId=2;
fogCover.freezeWorldMatrix();

function worldToFogPixel(x,z){
  const u=B.Scalar.Clamp(
    (x-FOG_WORLD_MIN_X)/FOG_WORLD_W,
    0,
    1
  );
  // Canvas Y crece hacia abajo; invertimos Z para conservar orientación del mapa.
  const v=B.Scalar.Clamp(
    (FOG_WORLD_MAX_Z-z)/FOG_WORLD_D,
    0,
    1
  );
  return {
...
function clearFogAtWorld(x,z,radius=FOG_CLEAR_RADIUS){
  const p=worldToFogPixel(x,z);

  // Convertimos el radio del mundo a píxeles X/Y.
  const rx=(radius/FOG_WORLD_W)*FOG_TEX_W;
  const ry=(radius/FOG_WORLD_D)*FOG_TEX_H;

  fmctx.save();
  fmctx.globalCompositeOperation='destination-out';
  fmctx.translate(p.x,p.y);
  fmctx.scale(rx,ry);

  // Centro 100% limpio; borde pequeño y suave.
  const g=fmctx.createRadialGradient(0,0,0,0,0,1);
  g.addColorStop(0,'rgba(0,0,0,1)');
  g.addColorStop(.82,'rgba(0,0,0,1)');
  g.addColorStop(1,'rgba(0,0,0,0)');

  fmctx.fillStyle=g;
  fmctx.beginPath();
  fmctx.arc(0,0,1,0,Math.PI*2);
  fmctx.fill();
  fmctx.restore();
...
let lastDiscoveryPoint=null;

function updateDiscoveryMask(){
  const x=player.position.x;
  const z=player.position.z;

  // Garantía absoluta: el círculo inmediato SIEMPRE está limpio.
  clearFogAtWorld(x,z,FOG_CLEAR_RADIUS);

  const point=new B.Vector3(x,0,z);
  if(
    !lastDiscoveryPoint ||
    B.Vector3.DistanceSquared(point,lastDiscoveryPoint)>.24*.24
  ){
    discoveredPath.push({x,z});
    if(discoveredPath.length>1100)discoveredPath.shift();
    lastDiscoveryPoint=point.clone();

    // Cada paso queda grabado permanentemente en la máscara.
    clearFogAtWorld(x,z,FOG_CLEAR_RADIUS);
  }

  // El dibujo del vapor se mueve muy lentamente,
  // pero la máscara descubierta NO se mueve.
  const t=performance.now()/1000;
  fogSmokeTex.uOffset=Math.sin(t*.035)*.035;
  fogSmokeTex.vOffset=Math.cos(t*.028)*.028;
}

// Descubrir inmediatamente el punto inicial cuando el jugador esté listo.
...
// ---------------------------------------------------------
// DESCUBRIMIENTO DEL MAPA v25
// ---------------------------------------------------------
// Compatibilidad con las funciones de guía existentes.
const clearedTrail=[];
let lastTrailPoint=null;

function recordClearedTrail(){
  const p=new B.Vector3(player.position.x,0,player.position.z);
  if(!lastTrailPoint || B.Vector3.DistanceSquared(p,lastTrailPoint)>.24*.24){
    clearedTrail.push({x:p.x,z:p.z});
    if(clearedTrail.length>1100)clearedTrail.shift();
    lastTrailPoint=p.clone();
    clearFogAtWorld(p.x,p.z,FOG_CLEAR_RADIUS);
  }
}

function puffInsideClearedTrail(wx,wz){
  for(let i=clearedTrail.length-1;i>=0;i--){
    const p=clearedTrail[i];
    const dx=wx-p.x,dz=wz-p.z;
    if(dx*dx+dz*dz<FOG_CLEAR_RADIUS*FOG_CLEAR_RADIUS)return true;
  }
  return false;
}
...
const camera=new B.FreeCamera('camera',new B.Vector3(0,24,6.8),scene);
camera.inputs.clear();
camera.fov=0.82;
camera.minZ=.08;
camera.maxZ=220;
...
// Resalte del personaje para que nunca se pierda entre las tinieblas.
const playerHighlight=new B.HighlightLayer('playerHighlight',scene);
playerHighlight.blurHorizontalSize=0.035;
playerHighlight.blurVerticalSize=0.035;
...
function updateDarkness(){
  ...
  // La capa FogOfWarCover es la responsable de ocultar el mapa.
  // El fog de Babylon solo suaviza la distancia extrema.
  scene.fogStart=54.0;
  scene.fogEnd=B.Scalar.Clamp(105.0-low*10.0,88.0,105.0);

  cloudDark.alpha=B.Scalar.Lerp(.30,.42,low);
  cloudMid.alpha=B.Scalar.Lerp(.22,.34,low);
  cloudLight.alpha=B.Scalar.Lerp(.14,.24,low);

  stormEmitter.position.set(player.position.x,player.position.y,player.position.z);

  if(performance.now()>nextNaturalLightning){
    nextNaturalLightning=performance.now()+6200+rand()*7800;
    flash('#eef7ff',60,.52);
    setTimeout(()=>flash('#c9deed',45,.22),90);
    if(audioCtx){
      setTimeout(()=>{tone(54,.16,'triangle',.02);},420+rand()*780);
    }
  }
}
  ...
  try{
    ...
    meshes.forEach(m=>{
      ...
      try{
        ...
        m.edgesWidth=1.8;
        m.edgesColor=new B.Color4(.01,.025,.055,.78);
      }catch(_){}
      try{
        playerHighlight.addMesh(m,new B.Color3(.18,.55,.86));
      }catch(_){}
    });
    ...
    setStatus(`TINIEBLAS TOTALES v25 · ${groundBoneMode} · ${boneNames}`);
    loadingEl.classList.add('hidden');

    // El personaje inicia dentro de un área totalmente limpia.
    clearFogAtWorld(player.position.x,player.position.z,FOG_CLEAR_RADIUS+0.35);
    lastDiscoveryPoint=new B.Vector3(player.position.x,0,player.position.z);
...
const baseCamX=player.position.x*.52;
const baseCamY=24.0;
const baseCamZ=player.position.z+6.5;
const lookZ=player.position.z-4.7;

if(quakeTimer>0){
  quakeTimer-=dt;
  const q=.13*Math.min(1,quakeTimer);
  camera.position.set(
...
camera.setTarget(new B.Vector3(player.position.x*.18,.35,lookZ));
});
