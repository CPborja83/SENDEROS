...
CROSS_STREETS.forEach((z,i)=>{
  box('crossRoad'+i,54,.12,2.15,0,-.105,z,mats.road);
...
makeHouse(-9.0,12.0,'CASA FAMILIAR',mats.wall1);
makeHouse( 9.0,11.0,'CASA FAMILIAR',mats.wall3);
...
// ---------------------------------------------------------
function makeChurch(){
  const x=-14.4,z=-12.5,w=13.8,d=10.5;
...
function makeLibrary(){
  const x=14.4,z=-12.5,w=13.8,d=10.5;
  openShell('Library',x,z,w,d,mats.wall2,mats.floorBlue,2.0);
...
function makeDentist(){
  const x=-14.5,z=-25.4,w=9.6,d=8.0;
  openShell('Dentist',x,z,w,d,mats.wall3,mats.floorGreen,1.6);
...
function makeHospital(){
  const x=14.0,z=-31.7,w=18.0,d=20.5;
  openShell('Hospital',x,z,w,d,mats.wall1,mats.floorGreen,2.6);
...
building(-8.7,-39.5,5.0,5.2,1.5,mats.wall2,true);
building( 8.5,-45.0,5.1,5.1,1.5,mats.wall3,true);
...
function makeMistPatch(x,z,id){
  ...
  for(let i=0;i<5;i++){
    ...
    plane.parent=root;
    plane.position.set((rand()-.5)*2.4,.55+rand()*1.45,(rand()-.5)*2.2);
    plane.material=mistMat;
    plane.billboardMode=B.Mesh.BILLBOARDMODE_Y;
    plane.isPickable=false;
    plane.visibility=.74+rand()*.20;
    plane.metadata={
      baseY:plane.position.y,
      phase:rand()*Math.PI*2,
      baseScale:0.82+rand()*.38
    };
...
// Tinieblas repartidas por todo el mapa, incluyendo calles e interiores no explorados.
let mistId=0;
for(let z=-55;z<=22;z+=5.4){
  for(let x=-28;x<=28;x+=5.4){
    if(rand()<.08)continue;
    makeMistPatch(x+(rand()-.5)*1.1,z+(rand()-.5)*1.1,mistId++);
  }
}
...
function updateCloudClearing(){
  ...
  // Las tinieblas se mueven como vapor vivo, pero desaparecen
  // permanentemente donde el personaje ya caminó.
  for(const patch of mistPatches){
    ...
    const aroundPlayer=(dx*dx+dz*dz)<5.0*5.0;
    const oldTrail=puffInsideClearedTrail(bx,bz);

    if(aroundPlayer || oldTrail){
      patch.metadata.cleared=true;
      patch.setEnabled(false); // despeje TOTAL, inmediato y permanente
      continue;
    }

    // Deriva lenta, tipo vapor; nunca parece una piedra estacionaria.
    const phase=patch.metadata.phase||0;
    patch.position.x=bx+Math.sin(t*.18+phase)*.48;
    patch.position.z=bz+Math.cos(t*.13+phase)*.34;
    patch.position.y=Math.sin(t*.16+phase)*.09;

    for(const layer of patch.getChildMeshes(false)){
      const lm=layer.metadata||{};
      layer.position.y=(lm.baseY||.8)+Math.sin(t*.42+(lm.phase||0))*.16;
      const pulse=(lm.baseScale||1)*(1+Math.sin(t*.31+(lm.phase||0))*.08);
      layer.scaling.x=B.Scalar.Lerp(layer.scaling.x,pulse,.06);
      layer.scaling.y=B.Scalar.Lerp(layer.scaling.y,pulse*.92,.06);
    }
  }
}
...
const camera=new B.FreeCamera('camera',new B.Vector3(0,34,9),scene);
camera.inputs.clear();
camera.fov=1.04;
camera.minZ=.08;
camera.maxZ=220;
...
function updateDarkness(){
  ...
  // La cámara permite comprender el mapa; las tinieblas físicas ocultan zonas sin explorar.
  scene.fogStart=28.0;
  scene.fogEnd=B.Scalar.Clamp(72.0-low*9.0,58.0,72.0);

  // Vapor más denso cuando la atención disminuye.
  mistMat.alpha=B.Scalar.Lerp(.72,.90,low);
  cloudDark.alpha=B.Scalar.Lerp(.42,.58,low);
  cloudMid.alpha=B.Scalar.Lerp(.34,.50,low);
  cloudLight.alpha=B.Scalar.Lerp(.24,.38,low);
...
async function loadCharacter(){
  try{
    ...
    setStatus(`CIUDAD VIVA v19 · ${groundBoneMode} · ${boneNames}`);
...
scene.onBeforeRenderObservable.add(()=>{
  ...
  let nextX=B.Scalar.Clamp(player.position.x+velocity.x*dt,-21.5,21.5);
  ...
  // Vista dollhouse/cenital: se ven habitaciones completas y varias cuadras.
const baseCamX=player.position.x*.32; // sigue al jugador sin esconder todo el mapa
const baseCamY=34.0;
const baseCamZ=player.position.z+9.5;
const lookZ=player.position.z-7.5;
